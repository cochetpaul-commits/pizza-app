...
type Preview = { lignes: number; a_modifier: number; a_creer: number; prix_maj?: number; refs_maj?: number; offres_fermees?: number; changements: Chg[]; erreurs: Err[] };
...
export function ImportExportModal({ etabSlug, onClose, onDone }: { etabSlug: string; onClose: () => void; onDone: () => void }) {
  ...
  const [result, setResult] = useState<{ modifies: number; crees: number; prix_maj?: number; refs_maj?: number; offres_fermees?: number; echecs: Err[] } | null>(null);
        ...
        <section style={{ border: "1px solid #e5ddd0", borderRadius: 14, padding: 14 }}>
          ...
          {preview && (
            <div style={{ marginTop: 12 }}>
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 10 }}>
                ...
                {preview.offres_fermees ? <span style={{ fontSize: 13, color: "#8a2b2b" }}><strong>{preview.offres_fermees}</strong> fiche{preview.offres_fermees > 1 ? "s" : ""} inactive{preview.offres_fermees > 1 ? "s" : ""} dont les offres seront fermées</span> : null}
          ...
          {result && (
            <div style={{ marginTop: 12, background: result.echecs.length ? "#fbf0dc" : "#e6f2e9", borderRadius: 10, padding: "10px 12px", fontSize: 13 }}>
              <strong>{result.modifies}</strong> produit{result.modifies > 1 ? "s" : ""} modifié{result.modifies > 1 ? "s" : ""}{result.crees ? <>, <strong>{result.crees}</strong> créé{result.crees > 1 ? "s" : ""}</> : null}{result.prix_maj ? <>, <strong>{result.prix_maj}</strong> prix mis à jour</> : null}{result.refs_maj ? <>, <strong>{result.refs_maj}</strong> réf. / dates mises à jour</> : null}{result.offres_fermees ? <>, <strong>{result.offres_fermees}</strong> offre{result.offres_fermees > 1 ? "s" : ""} fermée{result.offres_fermees > 1 ? "s" : ""} (fiches inactives)</> : null}.
