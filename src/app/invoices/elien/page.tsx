...
export default function ElienInvoicePage() {
  ...
  async function handlePreview() {
    ...
    try {
      ...
      const res = await fetchApi("/api/invoices/elien", {
        method: "POST",
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "2rem" }}>
      <h1 style={{ fontSize: "1.5rem", fontWeight: 700, marginBottom: "1.5rem" }}>
        Import factures Eric Elien
      </h1>

      <div
        style={{ border: "2px dashed #ccc", borderRadius: 8, padding: "2rem", textAlign: "center", cursor: "pointer", marginBottom: "1rem", background: file ? "#f0fdf4" : "#fafafa" }}
        ...
        <input ref={fileInputRef} type="file" style={{ display: "none" }}
          onChange={(e) => {
            ...
            if (f && !f.name.toLowerCase().endsWith(".pdf") && f.type !== "application/pdf") {
              setError("Seuls les fichiers PDF sont acceptes"); return;
            }
            setFile(f); setPreview(null); setCommitResult(null); setError(null);
          }} />
        {file ? <p style={{ color: "#4a6741", fontWeight: 600 }}>{file.name}</p>
          : <p style={{ color: "#666" }}>Cliquez pour selectionner un PDF Eric Elien</p>}
      </div>

      {file && !preview && !commitResult && (
        <button onClick={handlePreview} disabled={loading}
          style={{ background: "#D4775A", color: "white", border: "none", borderRadius: 6, padding: "0.6rem 1.5rem", fontSize: "1rem", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.7 : 1 }}>
        ...
        <div style={{ marginTop: "1.5rem" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.875rem", marginBottom: "1.5rem" }}>
            <thead>
              <tr style={{ background: "#f1f5f9" }}>
                <th style={th}>Nom</th><th style={th}>Qte</th><th style={th}>PU HT</th><th style={th}>Total HT</th><th style={th}>TVA</th>
              </tr>
            </thead>
            <tbody>
          ...
          <div style={{ display: "flex", gap: "1rem" }}>
            <button onClick={handleCommit} disabled={loading}
              style={{ background: "#4a6741", color: "white", border: "none", borderRadius: 6, padding: "0.6rem 1.5rem", fontSize: "1rem", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.7 : 1 }}>
      ...
      {commitResult && (
        <div style={{ marginTop: "1.5rem", background: "#f0fdf4", border: "1px solid #86efac", borderRadius: 8, padding: "1.5rem" }}>
          ...
          <button onClick={() => { setFile(null); setPreview(null); setCommitResult(null); setError(null); }}
            style={{ marginTop: "1rem", background: "#D4775A", color: "white", border: "none", borderRadius: 6, padding: "0.6rem 1.5rem", fontSize: "1rem", cursor: "pointer" }}>
