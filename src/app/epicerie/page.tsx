  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
      <button onClick={() => onChange(Math.max(min, Math.round((value - step) * 100) / 100))}
        style={{ width: 24, height: 24, borderRadius: 4, border: "1px solid #d1d5db", background: "#f9f9f9", cursor: "pointer", fontWeight: 900, fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center" }}>−</button>
...
export default function EpiceriePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: ings }, { data: sups }, { data: offs }] = await Promise.all([
        supabase.from("ingredients").select("id,name,category,cost_per_unit,piece_weight_g,piece_volume_ml,default_unit").eq("is_active", true).order("name"),
        supabase.from("suppliers").select("id,name").eq("is_active", true).order("name"),
        supabase.from("v_latest_offers").select("ingredient_id,supplier_id,unit,unit_price,pack_price,pack_total_qty,pack_unit,pack_count,pack_each_qty,pack_each_unit,density_kg_per_l,piece_weight_g"),
      ]);
  ...
  const btnPrimary = { padding: "7px 16px", borderRadius: 8, background: "#8B1A1A", color: "#fff", fontWeight: 800, fontSize: 13, border: "none", cursor: "pointer" as const };
  const btnSecondary = { padding: "7px 16px", borderRadius: 8, background: "transparent", color: "#8B1A1A", fontWeight: 800, fontSize: 13, border: "1px solid #8B1A1A", cursor: "pointer" as const };
  const btnGhost = (active?: boolean) => ({ padding: "5px 12px", borderRadius: 8, background: active ? "#8B1A1A" : "transparent", color: active ? "#fff" : "#6B6257", fontWeight: 700, fontSize: 12, border: "1px solid #d1d5db", cursor: "pointer" as const });
                  ...
                  return (
                    <div key={ing.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "7px 10px", borderRadius: 8, marginBottom: 3, background: inCalc ? "#FEF3E8" : "#FAF7F2", border: `1px solid ${inCalc ? "#FBBF24" : "#F0EBE0"}` }}>
                      ...
                      <button onClick={() => addToCalc(ing)} disabled={inCalc || !hasPrice}
                        style={{ marginLeft: 8, width: 26, height: 26, borderRadius: 6, border: "none", background: inCalc ? "#FBBF24" : hasPrice ? "#8B1A1A" : "#E5E7EB", color: inCalc ? "#fff" : hasPrice ? "#fff" : "#9CA3AF", fontWeight: 900, fontSize: 15, cursor: inCalc || !hasPrice ? "default" : "pointer" }}>
                        {inCalc ? "✓" : "+"}
                      </button>
                    </div>
                  );
            ...
            {lines.length === 0 ? (
              ...
              <div style={{ ...card, overflow: "hidden", padding: 0 }}>
                    ...
                    <tbody>
                      {lines.map((l, i) => (
                        <tr key={l.id} style={{ borderBottom: "1px solid #F0EBE0", background: i % 2 === 0 ? "#fff" : "#FDFCFA" }}>
                          ...
                          <td style={{ padding: "4px 10px", textAlign: "right" }}>
                            <span style={{ fontWeight: 900, fontSize: 14, color: "#8B1A1A" }}>{l.priceRounded.toFixed(2)} €</span>
                          </td>
                          <td style={{ padding: "4px 10px", textAlign: "right" }}>
                ...
                <div style={{ padding: "10px 16px", borderTop: "1px solid #E8E0D0", background: "#FAF7F2", display: "flex", justifyContent: "space-between", fontSize: 12, color: "#9B8E7E" }}>
                  ...
                  <span>Marge moy. : <strong style={{ color: "#8B1A1A" }}>{(lines.reduce((a, l) => a + l.margin, 0) / lines.length).toFixed(1)}%</strong></span>
                </div>
              </div>
            )}
          </div>
