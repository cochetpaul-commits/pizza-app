...
import { parseBarSpiritsInvoiceText, type ParsedInvoice } from "@/lib/invoices/barspirits";
...
async function pdfToTextWithPoppler(pdfBytes: Uint8Array): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "pizzaapp-barspirits-"));
...
export async function POST(req: Request) {
  try {
    ...
    if (!name.toLowerCase().endsWith(".pdf")) {
      return NextResponse.json({ ok: false, error: "Seuls les .pdf sont supportés sur BAR SPIRITS." }, { status: 400 });
    }
    ...
    const payload: ParsedInvoice = parseBarSpiritsInvoiceText(text);
    ...
    const supplierName = "BAR SPIRITS";
    ...
    if (!supplierId) throw new Error("Supplier BAR SPIRITS: id manquant");
    ...
    if (lines.length) {
      ...
      for (const l of lines) {
        ...
        if (!nm) continue;
        if (nameToIngId.has(nm.toLowerCase())) continue;
        toCreate.push({
          user_id: userId,
          name: nm,
      ...
      const offerCandidates = lines
        .map((l) => {
          const nm = (l.name ?? "").trim();
          const ingId = nameToIngId.get(nm.toLowerCase()) || null;
