...
export async function POST(req: Request) {
  try {
    ...
    const payload = parseSumInvoiceText(text);
    ...
    const result = await runImport({
      supabase, userId, supplierName: "SUM",
      payload, sourceFileName: file.name, rawText: text, mode, establishment,
      defaultUnit: "g", etabId,
      filterLine: (l) => l.notes !== "RUPTURE",
    });

    return NextResponse.json({
      ok: true, kind: "sum", filename: file.name, bytes: bytes.byteLength,
