...
import { parseSumInvoiceText, type ParsedInvoice } from "@/lib/invoices/sum";
...
async function pdfToTextWithPoppler(pdfBytes: Uint8Array): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "pizzaapp-sum-"));
...
export async function POST(req: Request) {
  try {
    ...
    if (!name.toLowerCase().endsWith(".pdf")) {
      return NextResponse.json({ ok: false, error: "Seuls les .pdf sont supportés sur SUM." }, { status: 400 });
    }
    ...
    const payload: ParsedInvoice = parseSumInvoiceText(text);
    ...
    if (!supplierId) throw new Error("Supplier SUM: id manquant");
    ...
    // Only process deliverable lines (not RUPTURE)
    const lines = (payload.lines ?? []).filter(
      (l) => (l.name ?? "").trim().length > 0 && l.notes !== "RUPTURE"
    );
    ...
    if (lines.length) {
      ...
      // Deduplicate by ingredient (keep last price seen for each ingredient)
