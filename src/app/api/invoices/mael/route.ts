...
import { parseMaelInvoiceText, type ParsedInvoice } from "@/lib/invoices/mael";
...
async function pdfToTextWithPoppler(pdfBytes: Uint8Array): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "pizzaapp-mael-"));
...
export async function POST(req: Request) {
  try {
    ...
    const payload: ParsedInvoice = parseMaelInvoiceText(text);
