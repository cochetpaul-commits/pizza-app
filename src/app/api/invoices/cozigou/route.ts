...
export async function POST(req: Request) {
  try {
    ...
    const text = await pdfToText(bytes, { prefix: "pizzaapp-cozigou-", flags: ["-raw"] });
    const payload = parseCozigouInvoiceText(text);
    ...
    const result = await runImport({
      supabase, userId, supplierName: "COZIGOU",
      payload, sourceFileName: file.name, rawText: text, mode, establishment,
      defaultUnit: "g",
    });

    return NextResponse.json({
      ok: true, kind: "cozigou", filename: file.name, bytes: bytes.byteLength,
