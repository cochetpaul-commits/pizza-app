...
import { parseCozigouInvoiceText, type ParsedInvoice } from "@/lib/invoices/cozigou";
...
async function pdfToTextWithPoppler(pdfBytes: Uint8Array): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "pizzaapp-cozigou-"));
...
export async function POST(req: Request) {
  try {
    ...
    if (!name.toLowerCase().endsWith(".pdf")) {
      return NextResponse.json({ ok: false, error: "Seuls les .pdf sont supportés sur COZIGOU." }, { status: 400 });
    }
    ...
    const payload: ParsedInvoice = parseCozigouInvoiceText(text);
    ...
    if (!supplierId) throw new Error("Supplier COZIGOU: id manquant");
    ...
    if (lines.length) {
      ...
      // Recharger les IDs après création
      if (skus.length) {
