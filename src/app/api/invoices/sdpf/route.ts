...
export async function POST(req: Request) {
  try {
    ...
    if (!file.name.toLowerCase().endsWith(".pdf")) {
      return NextResponse.json({ ok: false, error: "Seuls les .pdf sont supportés sur SDPF." }, { status: 400 });
    }

    const bytes = new Uint8Array(await file.arrayBuffer());
    const text = await pdfToText(bytes);
    const payload = parseSdpfInvoiceText(text);
    ...
    const result = await runImport({
      supabase, userId, supplierName: "SDPF",
      payload, sourceFileName: file.name, rawText: text, mode, establishment,
      defaultUnit: "kg", etabId,
    });

    return NextResponse.json({
      ok: true, kind: "sdpf", filename: file.name, bytes: bytes.byteLength,
