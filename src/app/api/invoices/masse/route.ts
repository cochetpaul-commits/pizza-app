...
export async function POST(req: Request) {
  try {
    ...
    if (!file.name.toLowerCase().endsWith(".pdf")) {
      return NextResponse.json({ ok: false, error: "Seuls les .pdf sont supportés sur MASSE." }, { status: 400 });
    }

    const bytes = new Uint8Array(await file.arrayBuffer());
    const text = await pdfToText(bytes);
    const payload = parseMasseInvoiceText(text);
    ...
    const result = await runImport({
      supabase, userId, supplierName: "MASSE",
      payload, sourceFileName: file.name, rawText: text, mode, establishment,
      defaultUnit: "kg", etabId,
    });

    return NextResponse.json({
      ok: true, kind: "masse", filename: file.name, bytes: bytes.byteLength,
