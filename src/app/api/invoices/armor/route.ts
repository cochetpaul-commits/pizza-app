...
import { parseArmorInvoiceText, type ParsedInvoice } from "@/lib/invoices/armor";
...
async function pdfToTextWithPoppler(pdfBytes: Uint8Array): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "pizzaapp-armor-"));
...
export async function POST(req: Request) {
  try {
    ...
    if (!name.toLowerCase().endsWith(".pdf")) {
      return NextResponse.json({ ok: false, error: "Seuls les .pdf sont supportés sur ARMOR." }, { status: 400 });
    }
    ...
    const payload: ParsedInvoice = parseArmorInvoiceText(text);
    ...
    const supplierName = "ARMOR EMBALLAGES";
    ...
    if (!supplierId) throw new Error("Supplier ARMOR EMBALLAGES: id manquant");
...

    {
      ...
      if ((linesCount ?? 0) === 0) {
        const lineInserts = payload.lines.map((l) => ({
          ...
          notes: l.tax_rate != null ? `TVA=${l.tax_rate}` : null,
        }));
    ...
    if (mode !== "commit") {
      return NextResponse.json({
        ...
        inserted: { supplier_id: supplierId, ingredients_created: 0, offers_inserted: 0 },
        parsed: payload,
      });
    }
    ...
    if (lines.length) {
      ...
      // Deduplicate by ingredient (keep last price seen)
