...
import { parseVinofloInvoiceText, type ParsedInvoice } from "@/lib/invoices/vinoflo";
...
async function pdfToTextWithPoppler(pdfBytes: Uint8Array): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "pizzaapp-vinoflo-"));
...
export async function POST(req: Request) {
  try {
    ...
    const payload: ParsedInvoice = parseVinofloInvoiceText(text);
    ...
    if (!supplierId) throw new Error("Supplier VINOFLO: id manquant");
    ...
    // Filter out alcohol tax lines — they're not ingredients
    const lines = (payload.lines ?? []).filter(
      (l) => (l.name ?? "").trim().length > 0 && l.notes !== "taxe_alcool"
    );
    ...
    if (lines.length) {
      ...
      const offerCandidates = lines
        .map((l) => {
          ...
          const nm = (l.name ?? "").trim();
          const ingId =
            (sku && skuToIngId.get(sku)) || nameToIngId.get(nm.toLowerCase()) || null;
