...
export async function POST(req: Request) {
  try {
    ...
    const text = await pdfToText(bytes, { prefix: "pizzaapp-metro-", flags: ["-raw"] });
    const payload = parseMetroInvoiceText(text);
    ...
    const result = await runImport({
      supabase, userId, supplierName: "METRO",
      payload, sourceFileName: file.name, rawText: text, mode, establishment,
      defaultUnit: "g",
    });

    return NextResponse.json({
      ok: true, kind: "metro", filename: file.name, bytes: bytes.byteLength,
