      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerRight}>
            ...
            <Text style={styles.docType}>Fiche Technique — Cocktail</Text>
          </View>
        </View>
        ...
        <View style={styles.topSection}>
          <View style={styles.topLeft}>
            <Text style={styles.recipeName}>{data.cocktailName}</Text>
            <Text style={styles.recipeSubtitle}>Cocktail — Fiche technique</Text>
            <View style={styles.infoBox}>
              ...
              <View style={styles.infoRow}>
                <Text style={styles.infoLabel}>Verre</Text>
                <Text style={styles.infoValue}>{glassLabel}</Text>
              </View>
              {data.garnish ? (
                <View style={styles.infoRow}>
                  <Text style={styles.infoLabel}>Garniture</Text>
                  <Text style={styles.infoValue}>{data.garnish}</Text>
                </View>
              ) : null}
              <View style={styles.infoRow}>
                <Text style={styles.infoLabel}>Coût matière</Text>
                <Text style={styles.infoValue}>{fmtMoney(data.totalCost)}</Text>
              </View>
              {data.sellPrice != null && data.sellPrice > 0 ? (
                <View style={styles.infoRow}>
                  <Text style={styles.infoLabel}>Prix de vente</Text>
                  <Text style={styles.infoValue}>{fmtMoney(data.sellPrice)}</Text>
                </View>
              ) : null}
            </View>
          </View>
        ...
        <View style={styles.tableHeader}>
          ...
          <Text style={[styles.th, styles.colCost]}>Coût</Text>
        </View>
        {data.lines.map((i, idx) => {
          ...
          return (
            <View key={idx} style={[styles.row, idx % 2 === 1 ? styles.rowEven : {}]}>
              ...
              <Text style={styles.colCost}>{fmtMoney(i.cost)}</Text>
            </View>
          );
        })}
        ...
        <Text style={styles.notesText}>{data.steps || "—"}</Text>
