...
/**
 * Import de la base produits depuis les lignes d'un classeur (feuille Produits de l'export).
 * Séparé de la route HTTP pour être testable à sec (mode preview) : scripts/test-import-produits.ts.
 *  - preview : liste des changements champ par champ, rien n'est écrit
 *  - commit  : application (fiches, réf. fournisseur, fournisseur des nouvelles fiches, offres)
 */
export async function importerClasseur(sheetRows: Record<string, unknown>[], mode: string, etabDefaut: string, userId: string | null): Promise<ResultatImport> {
  ...
  sheetRows.forEach((row, idx) => {
    ...
    // Réf. modifiée (avec ou sans colonnes de prix) : l'offre active du produit prend la même réf.
    {
      const skuCell = cellOf(row, "supplier_sku");
      const sku = skuCell === undefined ? null : (String(skuCell ?? "").trim() || null);
      const offre = id ? offerBy.get(id) : undefined;
      if (sku && offre && !(row as Record<string, unknown>).__prix && !(row as Record<string, unknown>).__offreMaj && sku !== ((offre.supplier_sku as string | null) ?? null)) {
        (row as Record<string, unknown>).__offreMaj = { offre_id: offre.id as string, sku, date: null };
        champs.prix_ref = { avant: offre.supplier_sku ? "réf. " + offre.supplier_sku : "sans réf.", apres: "réf. " + sku };
      }
    }
